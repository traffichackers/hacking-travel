...
var renderers = {
  ...
  renderRoads: function(traffic, roads, backgroundRoads, graphData) {
    ...
    var height = documentElement.height() - headerElement.height() - 30;
  ...
  renderMiseryIndex: function (traffic) {

    // Show speed and misery index
    var regionalConditions = helper.getMiseryIndex(traffic);
...
var helper = {
  ...
  getMiseryIndex: function(traffic) {
    var denominator = 0;
    var numerator = 0;
...
$.when(
  ...
  renderers.renderMiseryIndex(traffic);
  renderers.renderRoads(traffic, roadsResults[0], backgroundRoads[0], graphData);
